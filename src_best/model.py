...
# reference paper: Enhancing Sequential Recommendation with Graph Contrastive Learning
class Net(nn.Module):
    ...
    def forward(self, ori_item_list, aug_item_list):
        ...
        aug_global_sum = torch.sum(aug_global * ori_mask_item.unsqueeze(-1), dim=1) / torch.sum(ori_mask_item.unsqueeze(-1).float(), dim=1)
        ...
        # print("")
        # print('ori_local shape:\t{}\taug_local shape:\t{}'.format(ori_local.shape, aug_local.shape))
        # print('ori_global shape:\t{}\taug_global shape:\t{}'.format(ori_global.shape, aug_global.shape))
        # print('ori_global_sum shape:\t{}\taug_global_sum shape:\t{}\t'.format(ori_global_sum.shape, aug_global_sum.shape))
        # return shape : [batch_size, max_seq_length, hidden_size]
        return ori_local, aug_local, ori_global_sum, aug_global_sum
    ...
    def gnn_encode(self, items):
        ...
        edge_index = dropout_edge(self.global_graph.edge_index, p=self.args.edge_drop)[0]
        subgraph_loaders = NeighborSampler(edge_index, node_idx=items, sizes=self.args.neig_sample,
